...
class Instrument():
    ...
    def _performCommand(self, functioncode, payloadToSlave):
        ...
        """
        DEFAULT_NUMBER_OF_BYTES_TO_READ = 100
        ...
        if self.precalculate_read_size:
            try:
                ...
                if self.debug:
                    template = 'Debug mode. Could not precalculate response size for Modbus {} mode. ' + \
    ...
    def _perform_custom_command(self, functioncode, payloadToSlave):
        """Performs the command having the *functioncode*.
        ...
        Returns:
            The extracted data payload from the slave (a string).
    ...
    def _communicate(self, request, number_of_bytes_to_read):
        ...
        if self.debug:
            _print_out('\nDebug mode. Writing to instrument (expecting {} bytes back): ({})'. \
                format(number_of_bytes_to_read, _hexlify(request)))
        ...
        if time_since_read < minimum_silent_period:
            ...
            if self.debug:
                template = 'Debug mode. Sleeping for {:.1f} ms. ' + \
            ...
            template = 'Debug mode. No sleep required before write. ' + \
        ...
        if self.handle_local_echo:
            ...
            if self.debug:
                template = 'Debug mode. Discarding this local echo: {!r} ({} bytes).' 
        ...
        if self.debug:
            template = 'Debug mode. Response from instrument:({}) ({} bytes), ' + \
